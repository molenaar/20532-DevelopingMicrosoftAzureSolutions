...
5.  In the **Solution Explorer** pane, expand the **Roles** solution folder.

6.  In the **Solution Explorer** pane, expand the **Contoso.Events.Worker** project.

7.  Double-click the **TableStorageHelper.cs** file.

8.  In the **TableStorageHelper** class, find the method with the following signature:

    ```
    IEnumerable<string> GetRegistrantNames(string eventKey);
    ```

9.  Remove the following single line of code in the class:

    ```
    return Enumerable.Empty<string>();
    ```

10.  At the end of the **GetRegistrantNames** method and before the closing parenthesis, create a **CloudTable** instance:
